...
> This is the repository's original README, inherited from the upstream
> [discord-bot-template](https://github.com/agubelu/discord-bot-template) by *agubelu*
> that senechal's bot scaffolding is based on. It is kept here for reference because it
> still documents the command/event **plugin pattern** and the `utils` helpers
> (`get_rel_path`, `get_emoji`, `get_channel`, `send_in_channel`, `try_upload_file`).
> The project-level README is now at [`/README.md`](../README.md).
>
> _Moved verbatim from `/README.md` on 2026-05-25 (Task 003). Some details (e.g.
> `settings.py`, `client.send_message`) describe the original template, not the current
> senechal code — see `documentation/01-code-review.md` for how the project works today._
